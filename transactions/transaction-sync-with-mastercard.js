...
const truelocalClient = require("../category/truelocal-client.js");
const mastercardClient = require("../category/mastercard-client.js");
...
//buscar en truelocal y mastercard
async function getCategoryByKeyword(keyword) {
  try {
    const categoryFound = await mastercardClient.search(keyword);
    console.log(">>>>>>>>>mastercard " + categoryFound);

    if (!categoryFound) {
      const categoryFound = await truelocalClient.search(keyword);
      console.log(">>>>>>>>>tl " + categoryFound);

      return await createKeywordCategory(categoryFound, keyword);
    }else{
      const mcccode = await mcccodes.getMCCCategoryByCode(categoryFound.substring(0,4));
      return mcccode.category;
    }
  } catch (err) {
